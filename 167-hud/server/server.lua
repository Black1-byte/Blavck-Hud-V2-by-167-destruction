...
Citizen.CreateThread(function()
    ...
    if Config.Framework == "esx" then
        frameworkObject.RegisterServerCallback("blvckhudv2:GetMoney", function(source, cb, moneytype)
            local src = source
...
RegisterServerEvent('blvckhudv2:RemoveItem')
AddEventHandler('blvckhudv2:RemoveItem', function(item, amount)
    local src = source
...
Citizen.CreateThread(function()
    ...
    for _,v in pairs(GetPlayers()) do
        ...
        if player ~= nil then
            ...
            TriggerClientEvent('blvckhudv2:client:UpdateSettings', v,  preferences[identifier])
            TriggerClientEvent('blackhudv2:SetForceHide', v, false)
            TriggerClientEvent('blvckhudv2:UpdateNitroData', v, nitro)
            TriggerClientEvent('blvckhudv2:Loaded', v)
        end
    end
end)
...
RegisterNetEvent('blvckhudv2:UpdateData')
AddEventHandler("blvckhudv2:UpdateData", function(settingstype, val)
    local src = source
...
RegisterServerEvent('blackhud-v2:server:EjectPlayer')
AddEventHandler('blackhud-v2:server:EjectPlayer', function(table, velocity)
   for i=1, #table do
        if table[i] then
            TriggerClientEvent("blackhud-v2:client:EjectPlayer", table[i], velocity)
        end
    end
end)
...
RegisterNetEvent('blvckhudv2:Loaded')
AddEventHandler('blvckhudv2:Loaded', function()
    playerLoaded = true
end)
...
RegisterNetEvent('blvckhudv2:OnAddedMoney')
AddEventHandler("blvckhudv2:OnAddedMoney", function(amount, minus, moneyType)
...
RegisterNetEvent('blackhudv2:SetForceHide')
AddEventHandler('blackhudv2:SetForceHide', function(val)
    while true do
        Citizen.Wait(0)
...
RegisterNetEvent('blvckhudv2:client:UpdateSettings')
AddEventHandler('blvckhudv2:client:UpdateSettings', function(data)
...
function CinematicShow(bool)
    ...
    TriggerEvent('blvckhudv2:client:UpdateSettings', preferencesData)
    if bool then
...
if Config.EnableSeatbelt then
    ...
    RegisterNetEvent('blackhudv2:seatbelt:toggle')
    AddEventHandler('blackhudv2:seatbelt:toggle', function(toggle)
    ...
    RegisterNetEvent('blackhud-v2:client:EjectPlayer')
    AddEventHandler('blackhud-v2:client:EjectPlayer', function(velocity)
	    ...
	    while true do
	        ...
	        if car ~= 0 and (wasInCar or IsCar(car)) then
                ...
                if speedBuffer[2] and GetEntitySpeedVector(car, true).y > 1.0  and speedBuffer[1] > 15 and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.255) then			   
                    ...
                    if #seatPlayerId > 0 then TriggerServerEvent("blackhud-v2:server:EjectPlayer", seatPlayerId, velBuffer[2]) end                    
	        	end
...
if Config.EnableEngineToggle then
    ...
    RegisterNetEvent('blackhud-v2:client:ToggleEngine')
    AddEventHandler('blackhud-v2:client:ToggleEngine', function(toggle)
        local vehicle = GetVehiclePedIsIn(playerPed, false)
        if vehicle == 0 or GetPedInVehicleSeat(vehicle, -1) ~= playerPed then return end
        if not toggle then
            Config.Notification(Config.Notifications["engine_off"]["message"], Config.Notifications["engine_off"]["type"])
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPauseMenuActive() and not pauseActive then
            pauseActive = true
            TriggerEvent('blackhudv2:SetForceHide', true)
        end
        if not IsPauseMenuActive() and pauseActive and not CinematicModeOn then
            pauseActive = false
            TriggerEvent('blackhudv2:SetForceHide', false)
        end
    end
end)